...
print("Running Extract-CredibleSNPsSet.R ....................................")
...
getCredible <- function(file) {
  ...
  # If the lead SNPs is not included in the credible set SNPs, add it to the list
  ...
  D <- cbind(rep(chr, length(indx)), A[indx, c("SNP", "R2", "P")], rep(leadSNP, length(indx)), rep(region, length(indx)))
  colnames(D) <- c("Chr", "SNP", "R2", "P", "leadSNP", "Region")
  ...
  D <- cbind(D[,"Chr"], pos-1, pos, D[,c("SNP", "R2", "P", "leadSNP", "Region")])
...
colnames(rslt) <- c("Chr", "Start", "Pos", "SNP", "R2", "P", "leadSNP", "Region")

rslt.bed <- rslt[, c("Chr", "Start", "Pos", "SNP", "leadSNP", "Region", "R2", "P")]

write.table(rslt.bed, file=paste(Work.Dir, "/BedData/", trait.type, "/credSNPs.bed", sep=""), sep="\t", quote=F, col.names=F, row.names=F, append=F)