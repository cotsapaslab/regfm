...
print("Running Rho-getCredible.R ....................................")
...
getCredible <- function(file) {
  ...
  rslt <- cbind(D[,"Chr"], pos-1, pos, D[,c("SNP", "R2", "P", "PPS", "leadSNP", "Region")])
  colnames(rslt) <- c("Chr", "Start", "Pos", "SNP", "R2", "P", "PPS", "leadSNP", "Region")
  
  rslt.bed <- rslt[, c("Chr", "Start", "Pos", "SNP", "leadSNP", "Region", "R2", "P", "PPS")]
  
  write.table(rslt.bed, file=paste0(Work.Dir, "/BedData/", trait.type, "/perLocus-credSNPs/", region), sep="\t", quote=F, col.names=F, row.names=F, append=F)
